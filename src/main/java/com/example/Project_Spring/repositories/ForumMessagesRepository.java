...
public interface ForumMessagesRepository extends JpaRepository<ForumMessages, Long> {
    ...
    @Query("select m from ForumMessages m where m.id_recipient = ?1")
    List<ForumMessages> findMessagesByRecipientId(Integer id);

    @Query("select m from ForumMessages m where m.id_sender = ?1")
    List<ForumMessages> findMessagesBySenderId(Integer id);

    @Query("select m from ForumMessages m where m.id_topic = ?1")
    List<ForumMessages> findMessagesByTopicId(Long id);

    @Query("select count(id_topic) from ForumMessages m where m.id_topic = ?1 ")
    ...
    @Query("select count(id_topic) from ForumMessages m")
